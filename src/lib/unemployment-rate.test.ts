...
import {
  getUnemploymentRateByAgeGroup,
  getUnemploymentRateBySex
...
describe('unempeloyment-rate', () => {
  ...
  test('should return average values', async () => {
    ...
    expect(categories).toEqual(
      expect.arrayContaining(['De 56 a 64 años', 'De 16 a 25 años'])
    )
  })
  ...
  test('should return value by sex', async () => {
    ...
    expect(index).toEqual('year')
    expect(categories).toEqual(
      expect.arrayContaining(['Ambos', 'Hombres', 'Mujeres'])
    )
  })
})
